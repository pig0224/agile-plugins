#!/usr/bin/env node
/**
 * agile-plugins 发版脚本（npm run release）
 *
 * 市场仓库推送即发版；本脚本在其上提供「带版本号的发布」：
 * 质量门（claude plugin validate）→ bump VERSION → tag → push，
  ...
  while (Date.now() - started < timeoutMs) {
    try {
      const res = await fetch(`${api}&created=>${new Date(started - 60_000).toISOString()}`, {
        headers: { 'User-Agent': 'agile-plugins-release' },
      });
      if (res.ok) {
...
async function main() {
  ...
  // ---------- 3. 质量门 ----------
  log(`▶ 质量门：claude plugin validate .`);
  if (DRY_RUN) {
    log('（dry-run 跳过）');
  } else {
    await sh('claude', ['plugin', 'validate', '.']);
  }
  ...
  if (conclusion === 'success') {
    ...
    log(`  用户侧拿到新版本：agile plugin install <name>（或 agile update --plugin）`);
