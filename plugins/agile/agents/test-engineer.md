...
2. **案例清单**（分「后端用例」「前端用例」两节；表格：TC-id、对应 AC、前置条件、步骤、期望结果、优先级 P0/P1/P2、类型 正常/边界/异常/e2e。**e2e = 浏览器端到端自动化，归入「前端用例」节**，只覆盖关键路径。开发期各自只在自己节内补充）
3. **数据准备**（测试数据构造说明）
4. **自动化映射**（分单测与 e2e 两类：单测对应仓库的测试文件；e2e 对应前端项目的 e2e 脚本（如 `e2e/*.spec.ts`）；不可自动化的标注手工步骤）
...
2. e2e 用例（标 `e2e` 的条目）：在前端项目执行其 e2e 脚本（如 `pnpm e2e`）；stage 环境已部署时指向 stage 做冒烟。项目未引入 e2e 框架时标「未执行」并注明缺口。
3. 按案例清单逐条核对结果，失败案例记录：现象、复现步骤、初步归因。
4. 汇总：通过率、失败清单、阻塞项、结论（通过验收 / 有条件通过 / 不通过）。
