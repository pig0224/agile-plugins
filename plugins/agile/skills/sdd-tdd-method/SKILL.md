...
5. **分工红线（不得反转）**：命令（主会话）负责前置校验与复核汇报，实施默认委派对应角色 subagent（subagent 拿不到主会话上下文——委派时必须显式传入任务编号、约束与验收要求，不得依赖「它应该知道」；subagent 产出必须经主会话按各命令的复核清单复核后才向用户汇报）。**禁止以「subagent 不可靠」「更快」「上下文更全」等理由擅自改由主会话直接实施、subagent 验收**。例外仅限命令文件显式声明的两类：①素材在主会话对话中的任务（信息汇总 / 交互收集类）；②**执行循环类命令**（backend / frontend / run-test / fix-bug）——主会话按对应角色规范（roles/*.md；run-test 用 agents/test-engineer.md 的 Stage 2 规范）直接执行，全程可见、可随时纠偏（架构决策，非模型可自选）。
