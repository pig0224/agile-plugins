...
| /agile:backend | 后端 TDD 开发与接口测试（主会话按角色规范直做） | 后端 |
| /agile:frontend | 前端分层开发与浏览器测试（主会话按角色规范直做） | 前端 |
...
| /agile:knowledge | 知识库建设与沉淀（build 辅助建设 / capture 从会话与过程产物提炼 / sync-template 组合模板耦合资产同步进抽屉） | 架构 |
| /agile:help | 本帮助 | - |
