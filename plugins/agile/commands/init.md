...
- **工作区导航检查**：workspace 根目录若无 `CLAUDE.md`（旧版 `agile init workspace` 不生成，存量工作区可能缺失）→ 记入第 ⑥ 步待办；已存在则跳过（人工维护内容不覆盖、不“帮忙整理”）
- **续接刚创建的项目**：扫 `projects/` **一层**（`projects/*/CLAUDE.md`——单例项目与组合模板的成员项目全部平铺于此），找团队规范段仍带「⛔ 栈领域待人工确认」标记的项目（= CLI 刚生成、未走完本命令定制的）；其技术栈节自带来源模板（如「模板 vue3-vite 生成」），无需额外元数据
  - 恰好一个 → 展示项目名与来源模板，经确认后**跳过第 ② 步，直接进 ③**
  - 多个 → 列出让用户挑
- **空项目骨架视同未定制项目**：`projects/` 下仅含 README.md 的空骨架不在续接识别范围——视同未定制项目，**不跳过生成**，走全新创建流程；选定**模板**生成时，先删除该空骨架目录（仅 README.md，无内容损失；非空目录 CLI 必报错、空目录放行）再执行 `agile init project`，落地目录名沿用原空骨架名（或经用户改定）
- **全新创建**：无候选 → 运行 `agile template list`，结合 registry 的 language / framework 字段（组合模板附成员清单）与用户需求交互选定——**单例模板**（可确定 `--name` 目录名，缺省用模板名，执行 ② 单例模板分支）或**组合模板**（执行 ② 组合分支）
...
```bash
agile init project --template <模板名> [--name <目录名>]
```

组合模板（先做撞名核对与成员目录命名问答，再生成）：

1. **事前撞名核对**：成员全部**平铺**落盘 `projects/<成员目录名>/`（成员名全局唯一，但落地目录名仍可能与 `projects/` 下已有项目撞名）——生成前列出有效成员目录名（默认 = 成员名，含自定义），扫 `projects/` 比对；有撞名 → 问用户（换 `--name` 覆盖名，或确认是补缺场景继续）
2. **成员目录命名问答**（预选项 = 组合定义的成员名，支持全默认一键通过）：每个成员可自定义落地目录名（如模板相关名长、落地想用短名）；说明成员项目包名 = 实际落地目录名（`{{name}}` = 目录名）。有自定义才拼 `--name`
3. 执行生成（成员目录已存在 → CLI 按生成清单校验：清单一致 → 跳过 + warn，组合定义演进后可后补成员，补缺按本次的有效成员目录名判定，覆盖过的成员须带相同 `--name` 键值；清单不符 → 硬错误，按提示删除该目录后重跑（2.4.0 起 `--force` 已删除）；无清单的陌生目录 = 同名普通项目 → 跳过 + warn，人工核对；空目录 → 放行生成）：

```bash
agile init project --template <组合名> [--name 成员名=目录名 ...]
```

（缺省 `--name` 各成员用组合项目名称落盘；输出汇报以组合名为标签；CLI 逐成员 git add 生成物；commit 时机由人工决定。）

### ③ 项目约定问答（≤2 轮 AskUserQuestion）

**组合模板场景**：③④⑤ 按**成员逐项目循环**（顺序依组合成员清单）；每轮提供「沿用上一成员口径」快捷项（同栈成员或全默认团队用）；各成员产物落在各自 `CLAUDE.md` / `docs/conventions.md` / `docs/ui.md`。
...
| UI 约定 | token 管理方式（CSS 变量 / JS token 文件 / 组件库主题配置）、主色/字号/圆角/间距基准、暗色模式是否需要（仅前端模板；回答写入 `docs/ui.md`） |
| 依赖选型 | 引入的组件库/关键依赖（供第 ⑤ 步检索框架 AI 能力） |

按回答改写 `docs/conventions.md`（「增长时的建议布局」→「本项目约定」等）与 `CLAUDE.md` 硬规则；UI 约定回答写入 `docs/ui.md`（前端模板预置骨架）；**未问到的维度保留模板默认值**（`docs/ui.md` 未问到处保留「待定」）。
...
- 生成物清单与问答结果摘要（哪些定制、哪些保留默认/待确认标记）；组合场景附**成员映射表**（成员 → 成员模板（组合专属）→ 实际目录）；有跳过成员时逐条列出 warn 并提示人工核对
- **工作区导航补齐**（若 ① 记录了待办）：经用户确认后，在 workspace 根生成 `CLAUDE.md` 导航地图——五类目录表按 `settings.paths` 实际路径、配置与清单指针（.agile/settings.json / manifests / solutions）、常用命令、AI 会话须知（含 `projects/<项目>/CLAUDE.md` 项目导航指针）；已存在则跳过。生成后提醒人工 git add
