...
- **全新创建**：无候选 → 运行 `agile template list`，结合 registry 的 language / framework 字段（组合模板附成员清单）与用户需求交互选定——**单模板**（确定项目名，执行 ② 单模板分支）或**组合模板**（确定系统标签，执行 ② 组合分支）
...
1. **事前撞名核对**：成员全部**平铺**落盘 `projects/<成员目录名>/`（成员名全局唯一，但落地目录名仍可能与 `projects/` 下已有项目撞名）——生成前列出有效成员目录名（默认 = 成员名，含自定义），扫 `projects/` 比对；有撞名 → 问用户（换 `--member` 覆盖名，或确认是补缺场景继续）
2. **成员目录命名问答**（预选项 = 组合定义的成员名，支持全默认一键通过）：每个成员可自定义落地目录名（如模板相关名长、落地想用短名）；说明成员项目包名 = 实际落地目录名（`{{name}}` = 目录名）。有自定义才拼 `--member`
3. 执行生成（成员目录已存在 → CLI 按生成清单校验：清单一致 → 跳过 + warn，组合定义演进后可后补成员，补缺按本次的有效成员目录名判定，覆盖过的成员须带相同 `--member`；清单不符 → 硬错误，按提示删除重跑或 `--force` 重生成；无清单的陌生目录 = 同名普通项目 → 跳过 + warn，人工核对）：

```bash
agile init project <系统标签> --template <组合名> [--member 成员名=目录名 ...]
```

（`<系统标签>` 仅为输出汇报，不落目录；CLI 逐成员 git add 生成物；commit 时机由人工决定。）
